...
class Gui:
    ...
    def __init__(self):
        ...
        self.character_cb = self.window.findChild(QComboBox,
                                                  "characterComboBox")
        ...
        # Connecting Signals
        self.character_cb.currentIndexChanged.connect(
        ...
        self.training_set = {'data': [], 'num': []}
        self.font_a = []
        self.populate_training_set('font_a.txt')

        self.perceptron = perceptron(80)
    ...
    def on_train_pushbutton_clicked(self):
        print(len(self.training_set['data']))
        self.perceptron.treinar(self.training_set['data'],
                                self.training_set['num'])
            ...
            for j in range(8):
                self.pixels.append(
                    self.window.findChild(QPushButton,
                                          "pixel" + str(i + 1) + str(j + 1)))
                self.pixels[-1].clicked.connect(
                    getattr(self, "on_pixel_" + str(i) + str(j) + "_clicked"))

    def populate_training_set(self, file):
        ...
        for line in f:
            if line.startswith("#"):
                nums.append(int(line.replace('#', '').replace('number', '')))
            else:
                aux = line.split(', ')
                for i in aux:
                    inputs[nums[-1]].append(int(i))

        for i in range(10):
            self.training_set['data'].append(inputs[i])
            self.training_set['num'].append(nums[i])
